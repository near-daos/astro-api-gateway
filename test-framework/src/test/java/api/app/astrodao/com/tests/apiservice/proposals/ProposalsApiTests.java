...
public class ProposalsApiTests extends BaseTest {
    ...
    void getListOfProposalsWithQueryParamsDaoStatusTypeProposer() {
        Map<String, Object> query = Map.of(
                "dao", testDao,
                "status", "InProgress",
                "type", "AddBounty",
    ...
    @Test
    @Severity(SeverityLevel.CRITICAL)
    @Story("Get list of proposals with query param: [failed=true]")
    @DisplayName("Get list of proposals with query param: [failed=true]")
    void getListOfProposalsWithQueryParamsFailed() {
        Map<String, Object> query = Map.of(
                "failed", true
        );
        ...
        proposalsApiSteps.assertDtoValueGreaterThan(proposalsResponse, r -> r.getTotal().intValue(), 50, "total");
        proposalsApiSteps.assertDtoValue(proposalsResponse, r -> r.getOffset().intValue(), 0, "offset");
        proposalsApiSteps.assertDtoValueGreaterThan(proposalsResponse, r -> r.getTotal().intValue(), 18855, "total");
        proposalsApiSteps.assertCollectionHasCorrectSize(proposalsResponse.getData(), 50);
        ...
        proposalsApiSteps.assertCollectionElementsHasValue(proposalsResponse.getData(), proposal -> proposal.getStatus() != Proposal.StatusEnum.APPROVED, "data/status");
        proposalsApiSteps.assertCollectionContainsAnyOf(proposalsResponse.getData(), Proposal::getVoteStatus, Arrays.asList(Proposal.VoteStatusEnum.values()), "data/voteStatus");
        proposalsApiSteps.assertCollectionElementsHasValue(proposalsResponse.getData(), r -> r.getKind() != null, "data/kind");
